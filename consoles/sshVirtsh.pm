...
sub to_byte {
    ...
    return $+{num} * $size_multiplier->{$+{unit} // 'b'};
}
...
sub add_disk {
    my ($self, $args) = @_;
    die('Use one out of `backingfile => 1`, `create => 1` or `cdrom => 1`') if ((($args->{backingfile} // 0) + ($args->{cdrom} // 0) + ($args->{create} // 0)) != 1);
    ...
    if ($self->vmm_family eq 'vmware') {
        ...
        if ($args->{create}) {
            ...
            my $cmd =
              ...
              "echo File $base_orig_file is being copied by other process, sleeping for 60 seconds; sleep 60;" .
              'done;' .
              'else ' .
              "cp /vmfs/volumes/$vmware_nfs_datastore/hdd/$base_orig_file $basedir;" .
              'fi;';
            ...
            my $cmd =
              ...
              "echo File $file is being copied by other process, sleeping for 60 seconds; sleep 60;" .
              'done;' .
              'else ' .
              "cp /vmfs/volumes/$vmware_nfs_datastore/iso/$file $basedir;" .
              'fi;';
            $self->run_cmd($cmd, domain => 'sshVMwareServer') && die "Can't copy VMware image $file";
        }
    } else {
